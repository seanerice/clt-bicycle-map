...
- **UI open question — reopened, not resolved.** `ui-layer.md` §9 asked whether route relations get clipped at the viewport edge. The layer docs originally landed on "no" (`ST_Intersects` without `ST_Intersection` — a route's full geometry returns whenever any part intersects the requested bbox), reasoning that clipping could make `cycling-route-symbols` label placement jump or duplicate across fetches. Revisited during PRD reconciliation (§6): Mapbox GL already clips *rendering* to the visible canvas regardless of whether the fetched geometry extends past it, so the original "line will look chopped up" framing doesn't hold — the real, narrower, untested cost of clipping is label-anchor stability, not line continuity. Left open rather than flipped — see §5/§6.
- **Stable feature id, mostly resolved.** `ui-layer.md` asked whether the API exposes a stable per-feature id; `api-layer.md` confirms yes (`osm_type`/`osm_id`, Contract B's own key), leaning toward GeoJSON `Feature.id`. Only the exact encoding is still open (§5 below).
...
- **GeoJSON `Feature.id` encoding for `osm_type`/`osm_id`** (api-layer.md §10, ui-layer.md §9) — small, not blocking, but worth a one-line decision before the API's response shape is implemented.
- **Clip route geometry to the query bbox, or keep returning it whole?** (api-layer.md §3, persistence-layer.md §2, ui-layer.md §9) — reopened during PRD reconciliation; see §6. Trades payload size for `cycling-route-symbols` label-placement stability, an untested risk rather than a demonstrated one. Needs a decision (or a "try clipping first" experiment) before Contract A's query is implemented for routes.
...
- **Named routes create a real tension between FR-3 and FR-5 — and it's the reason the clipping question above got reopened.** FR-3 wants continuous, unbroken route rendering; FR-5 wants payload scoped to the viewport. The originally-recorded "no clipping" answer satisfied FR-3 by always returning a route's full geometry once any part intersects the bbox — which api-layer.md §3 itself flags means "a route far larger than the viewport contributes its full geometry to every intersecting request's payload," directly working against FR-5 for any city-spanning route. Clipping to the query bbox would resolve the tension in FR-5's favor (payload actually stays viewport-scoped) without costing FR-3 anything visually, since Mapbox GL already clips rendering to the canvas regardless — the only real cost is the untested `cycling-route-symbols` label-stability risk noted above. Left as an open question rather than decided here.
