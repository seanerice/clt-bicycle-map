...
**Contract A — UI ↔ API.**
`GET /features?bbox=minLon,minLat,maxLon,maxLat&zoom=<map.getZoom()>` returns a GeoJSON `FeatureCollection`. The `zoom` param is **resolved 2026-08-21 (story 3.1): sent by the UI from day one**, even though the API doesn't act on it yet (api-layer.md §2 reserves the name; story 2.10 accepts-and-ignores it) — see ui-layer.md §9 item 2 and §5 below. Feature properties must keep the shape the frontend's style layers already key off (`website/src/bikemap-app.js`, `website/src/colors.js`):
