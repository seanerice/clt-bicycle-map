...
**Status: Phase 1 (stories 4.1–4.3) done 2026-08-29** — `scripts/pipeline/` package, `data/cities.json`, psycopg UPSERT into `features`, both transform bug fixes, the one-shot `ingest` compose service, and the `bikemap-ingest` systemd timer are all merged to `v2` (PRs #79/#80 → #82, plus #83) and live on the EC2 box; the first production ingest run (2026-08-29) loaded 6,008 features. Phase 2 (stories 4.4–4.9 — the Geofabrik extract + `osmium`/`osm2pgsql` pipeline that removes Overpass and delivers SM-1) is **not started**. The Goal/Scope below still describes the full epic (Phase 2's end state).
