...
| 5 | ~~Clip route geometry at the bbox, or return whole?~~ — **resolved: don't clip** (`ST_Intersects` only), per a real spike measuring 124,978 bytes unclipped vs. 63,601 bytes clipped for the largest currently-loaded route relation | Epic 2 (blocks finalizing the route query) | [architecture.md §6](./architecture.md#6-reconciliation-with-the-prd) |
