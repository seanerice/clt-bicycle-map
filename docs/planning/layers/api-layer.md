...
- ~~Clip route geometry to the bbox, or return it whole?~~ (§3) — **resolved 2026-08-20 (story 2.2): don't clip.** Decided against real measurements (Cross Charlotte Trail, the largest route relation currently loaded: 124,978 bytes unclipped vs. 63,601 bytes clipped for a bbox covering half its extent) plus the `cycling-route-symbols` `symbol-placement: line` label-anchor-stability risk under Epic 3's per-`moveend` refetch pattern. See §3 for the full writeup and architecture.md §5/§6 for the cross-cutting record.
