...
2. **Zoom hint to the API (§6).** Should the UI send `zoom` (or similar) alongside `bbox` from the start, even before the API acts on it? Recommended yes in §6, but it's a Contract A shape change and needs API-layer agreement before being treated as decided.
3. **Padding factor and debounce interval (§2.2, §2.3)** are placeholder numbers (200ms debounce, 25–50% pad) — fine to ship and tune, but flagging that they're guesses, not measurements, and should be revisited once there's a real API to measure latency against.
...
5. **API base URL / environment config.** `API_BASE_URL` in §2.4 needs to come from somewhere — webpack build-time env var, following whatever convention (if any) the API layer's deployment doc settles on. Not blocking for design but needs an answer before implementation; likely a `testing-and-tooling.md` §3 (deployment) concern more than a pure UI one.
