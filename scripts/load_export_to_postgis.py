...
loads the output of scripts/fetch_data.py (data/export.geojson) into the
`features` table created by the EF Core migrations in db/Migrations/, to
validate the schema described in docs/planning/layers/persistence-layer.md
end to end (in particular Contract B's UPSERT pattern, §5) before the real
ingestion loader exists. A future, unrelated epic replaces fetch_data.py's
write_data() with the real loader that writes straight to Postgres; this
script gets deleted once that lands. It is NOT wired into
.github/workflows/osm-refresh.yml and is not meant to be.

data/export.geojson is NOT committed to the repo (see .gitignore) — it's
build output, regenerated by fetch_data.py and synced to S3 by
osm-refresh.yml, never the map's actual source of truth. An earlier
committed copy sat in this repo unrefreshed from project inception until
it was deleted: by the time anyone checked, it predated fetch_data.py's
existence, was missing 3 of the 4 currently-covered towns, and lacked
properties (e.g. `highwayType`) the current pipeline always sets — i.e.
it silently stopped being representative of what this loader needs to
validate against. Run fetch_data.py first to produce a real one.

Usage (from repo root, with the `db` docker-compose service running,
...
and a freshly generated data/export.geojson):
