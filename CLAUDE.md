...
- `.github/workflows/osm-refresh.yml` — a PR-only pipeline smoke test (runs `python -m scripts.pipeline` for one small area against an ephemeral CI PostGIS). No cron, no S3 sync — the daily refresh runs on the EC2 box via the `bikemap-ingest` systemd timer (see below)
...
In production this runs on the EC2 box, not GitHub Actions: the `bikemap-ingest` systemd timer (`infra/backend/systemd/`, installed by `infra/backend/user-data.sh`) fires daily at 05:00 UTC and runs `docker compose -f docker-compose.yml -f docker-compose.prod.yml run --rm ingest --all` (the one-shot `ingest` service, `scripts/Dockerfile`). `.github/workflows/osm-refresh.yml` now only runs on PRs to `v2` (+ manual dispatch) as a one-area smoke test — no cron, no `aws s3 sync`.
...
There is no file output and no S3 sync anymore. The daily refresh runs on the EC2 box via the `bikemap-ingest` systemd timer (`infra/backend/systemd/`, installed by `infra/backend/user-data.sh`) → `docker compose ... run --rm ingest --all` (the one-shot `ingest` service, `scripts/Dockerfile`). `.github/workflows/osm-refresh.yml` now runs only on PRs to `v2` (+ manual dispatch), as a one-area pipeline smoke test against a CI PostGIS — no `schedule:`, no `aws s3 sync`.
